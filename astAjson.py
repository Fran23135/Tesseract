        ...
        while i < len(content):
            ...
            if bracket:
                ...
                if k == 'Value':
                    ...
                    result['limit'] = {'value': val(v)}
                    result['limit'].update(sub)
                elif k == 'CatchClause':
                    result['catchClause'] = {'exception': val(v)} if v else {}
                    result['catchClause'].update(re.sub)

                elif k == 'CatchType':
                    result['catchType'] = {'type': val(v)}
                    result['catchType'].update(re.sub)

                elif k == 'FinallyBlock':
                    result['finallyBlock'] = re.sub

                elif k == 'Extends':
                    result['extends'] = val(v)

                elif k == 'Implements':
                    result['implements'] = val(v)

                elif k == 'Modifier':
                    result['modifier'] = val(v)

                elif k == 'ClassMember':
                    if 'members' not in result:
                        result['members'] = []
                    result['members'].append(sub)

                elif k == 'AttributeDeclaration':
                    n = {'AttributeDeclaration': {'name': val(v)}}
                    for key in ['explicitType', 'dinamic', 'operation', 'longitud', 'limit']:
                        if key in sub.get('value', {}):
                            pass
                    if 'value' in sub:
                        for key in ['explicitType', 'dinamic', 'operation', 'longitud', 'limit']:
                            if key in sub:
                                sub['value'][key] = sub.pop(key)
                    if 'modifier' in sub:
                        n['AttributeDeclaration']['modifier'] = sub.pop('modifier')
                    n['AttributeDeclaration'].update(sub)
                    if 'members' not in result:
                        result['members'] = []
                    result['members'].append(n)

                elif k in ['AttributeConstantDeclaration', 'ConstantAttributeDeclaration']:
                    n = {k: {'name': val(v)}}
                    if 'value' in sub:
                        for key in ['explicitType', 'dinamic', 'longitud']:
                            if key in sub:
                                sub['value'][key] = sub.pop(key)
                    if 'modifier' in sub:
                        n[k]['modifier'] = sub.pop('modifier')
                    n[k].update(sub)
                    if 'members' not in result:
                        result['members'] = []
                    result['members'].append(n)

                elif k == 'MethodDeclaration':
                    n = {'MethodDeclaration': {'name': val(v)}}
                    if 'Parameters' in sub:
                        n['MethodDeclaration']['parameters'] = {'value': val(sub.pop('Parameters'))}
                    elif 'parameters' in sub and not isinstance(sub['parameters'], dict):
                        n['MethodDeclaration']['parameters'] = {'value': val(sub.pop('parameters'))}
                    else:
                        if 'parameters' in sub:
                            n['MethodDeclaration']['parameters'] = sub.pop('parameters')
                    if 'modifier' in sub:
                            n['MethodDeclaration']['modifier'] = sub.pop('modifier')
                    if 'explicitType' in sub:
                        n['MethodDeclaration']['explicitType'] = sub.pop('explicitType')
                    n['MethodDeclaration'].update(sub)
                    if 'members' not in result:
                        result['members'] = []
                    result['members'].append(n)

                elif k == 'ConstructorDeclaration':
                        n = {'ConstructorDeclaration': {'name': val(v)}}
                        if 'Parameters' in sub:
                            n['ConstructorDeclaration']['parameters'] = {'value': val(sub.pop('Parameters'))}
                        elif 'parameters' in sub and not isinstance(sub['parameters'], dict):
                            n['ConstructorDeclaration']['parameters'] = {'value': val(sub.pop('parameters'))}
                        else:
                            if 'parameters' in sub:
                                n['ConstructorDeclaration']['parameters'] = sub.pop('parameters')
                        if 'modifier' in sub:
                            n['ConstructorDeclaration']['modifier'] = sub.pop('modifier')
                        n['ConstructorDeclaration'].update(sub)
                        if 'members' not in result:
                            result['members'] = []
                        result['members'].append(n)
                elif k == 'VariableDeclarationStruct':
                        node = make('VariableDeclarationStruct', v, nested)
                        result['variableDeclarationStruct'] = node['VariableDeclarationStruct']
                elif k == 'ConstantDeclarationStruct':
                        node = make('ConstantDeclarationStruct', v, nested)
                        result['ConstantDeclarationStruct'] = node['ConstantDeclarationStruct']        
                elif k == 'Arguments':
                        result['arguments'] = {'value': val(v)}
                        if sub:
                            result['arguments'].update(sub)
                        else:
    ...
    def make(nt, nv, content):
        if nt == 'VariableDeclaration':
            ...
            if content:
                ...
                if 'NFunction' in parsed:
                    raw = str(parsed['NFunction'])
            ...
            if content:
                ...
                ordered = {}
                if 'block' in parsed:
                    ordered['tryBlock'] = parsed.pop('block')
                if 'catchClause' in parsed:
                    ordered['catchClause'] = parsed.pop('catchClause')
                if 'finallyBlock' in parsed:
                    ordered['finallyBlock'] = parsed.pop('finallyBlock')
                ordered.update(parsed)
                n['TryCatch'].update(ordered)
            return n

        elif nt == 'Throw':
            n = {'Throw': {'exception': nv}}
            if content:
                parsed = obj(content)
                if 'value' in parsed:
                    n['Throw']['value'] = parsed['value']
            return n

        elif nt == 'NewObject':
            n = {'NewObject': {'class': nv}}
            if content:
                parsed = obj(content)
                if 'Arguments' in parsed:
                    n['NewObject']['arguments'] = {'value': val(parsed.pop('Arguments'))}
                elif 'arguments' in parsed:
                    n['NewObject']['arguments'] = parsed.pop('arguments')
                n['NewObject'].update(parsed)
            return n

        elif nt == 'ClassDeclaration':
            n = {'ClassDeclaration': {'name': nv}}
            if content:
                parsed = obj(content)
                ordered = {}
                for key in ['modifier', 'extends', 'implements', 'members']:
                    if key in parsed:
                        ordered[key] = parsed.pop(key)
                ordered.update(parsed)
                n['ClassDeclaration'].update(ordered)
            return n

        elif nt == 'InterfaceDeclaration':
            n = {'InterfaceDeclaration': {'name': nv}}
            if content:
                parsed = obj(content)
                ordered = {}
                for key in ['extends', 'members']:
                    if key in parsed:
                        ordered[key] = parsed.pop(key)
                ordered.update(parsed)
                n['InterfaceDeclaration'].update(ordered)
            return n

        elif nt == 'ThisAccess':
            return {'ThisAccess': {'value': nv}}
            ...
            if content:
                parsed = obj(content)
                if 'Arguments' in parsed:
                    n['ThisCall']['arguments'] = {'value': val(parsed.pop('Arguments'))}
                elif 'arguments' in parsed:
                    n['ThisCall']['arguments'] = parsed.pop('arguments')
                n['ThisCall'].update(parsed)
            return n

        elif nt == 'ThisAssignment':
            n = {'ThisAssignment': {'value': nv}}
            if content:
                parsed = obj(content)
                if 'value' in parsed:
                    n['ThisAssignment']['assigned'] = parsed['value']
            return n

        elif nt == 'SuperAccess':
            return {'SuperAccess': {'value': nv}}

        elif nt == 'SuperCall':
            n = {'SuperCall': {'value': nv}}
            if content:
                parsed = obj(content)
                if 'Arguments' in parsed:
                    n['SuperCall']['arguments'] = {'value': val(parsed.pop('Arguments'))}
                elif 'arguments' in parsed:
                    n['SuperCall']['arguments'] = parsed.pop('arguments')
                n['SuperCall'].update(parsed)
            return n

        elif nt == 'SuperAssignment':
            n = {'SuperAssignment': {'value': nv}}
            if content:
                parsed = obj(content)
                if 'value' in parsed:
                    n['SuperAssignment']['assigned'] = parsed['value']
            return n

        elif nt == 'SuperConstructorCall':
            n = {'SuperConstructorCall': {'value': nv}}
            if content:
                parsed = obj(content)
                if 'Arguments' in parsed:
                    n['SuperConstructorCall']['arguments'] = {'value': val(parsed.pop('Arguments'))}
                elif 'arguments' in parsed:
                    n['SuperConstructorCall']['arguments'] = parsed.pop('arguments')
                n['SuperConstructorCall'].update(parsed)
            return n
        elif nt == 'VariableDeclarationStruct':
            n = {'VariableDeclarationStruct': {'name': nv}}
            if content:
                parsed = obj(content)
                if 'value' in parsed:
                    for key in ['explicitType', 'dinamic', 'operation', 'longitud',"limit"]:
                ...
                if 'longitud' in parsed:
                    ordered['longitud'] = parsed.pop('longitud')
                ordered.update(parsed)
                n['VariableDeclarationStruct'].update(ordered)
            return n
        elif nt == 'ConstantDeclarationStruct':
            n = {'ConstantDeclarationStruct': {'name': nv}}
            if content:
                parsed = obj(content)
                if 'value' in parsed:
                    for key in ['explicitType', 'dinamic', 'operation', 'longitud']:
                ...
                if 'longitud' in parsed:
                    ordered['longitud'] = parsed.pop('longitud')
                ordered.update(parsed)
                n['ConstantDeclarationStruct'].update(ordered)
            return n
        elif nt == 'StructDeclaration':
            n = {'StructDeclaration': {'name': nv}}
            if content:
                parsed = obj(content)
                if 'block' in parsed:
                    n['StructDeclaration']['block'] = parsed['block']
            return n

        elif nt == 'ModuleAsignement':
            n = {'ModuleAsignement': {'name': nv}}
            if content:
                parsed = obj(content)
                if 'value' in parsed and 'operation' in parsed:
                    parsed['value']['operation'] = parsed.pop('operation')
                n['ModuleAsignement'].update(parsed)
            return n
        else:
            return {nt: nv}
